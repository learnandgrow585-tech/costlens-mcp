...
const inputSchema = z.object({
  ...
  provider: z
    ...
    .optional()
    .describe("Filter to a specific cloud or AI provider (optional)"),
  period: z
    .enum(["current_month", "last_month", "all"])
    .optional()
    .default("current_month")
    .describe("Billing period to query (default: current_month)")
});
...
Understands questions like:
  ...
  - "Show me our OpenAI and Anthropic usage"
Returns a structured breakdown with totals, service-level detail, and a confidence score.
NOTE: Currently using FOCUS 1.2 sample data. Real adapters land in Week 4.`,
    inputSchema: {
      type: "object",
      properties: {
        question: {
  ...
  async call(args) {
    ...
    if (!parsed.success) {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ error: `Invalid input: ${issues}` }) }],
        isError: true
      };
    }

    const { question } = parsed.data;
    const result = queryCosts(question);
